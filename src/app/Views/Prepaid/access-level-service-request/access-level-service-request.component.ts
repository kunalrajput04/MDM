...
import { IServiceRequestAdd, UploadAtAccessLevel } from 'src/app/Model/iservice-request-add';
...
@Component({
  selector: 'app-access-level-service-request',
  templateUrl: './access-level-service-request.component.html',
  styleUrls: ['./access-level-service-request.component.css']
})
export class AccessLevelServiceRequestComponent implements OnInit {
  ...
  constructor(private router: Router, private userservice: UserService, private storeservice: SmartMeterService, private spinner: NgxSpinnerService, private toastr: ToastrService, private service: ServiceRequestService) { }
  ...
  onList() {
    ...
    this.service.getConsumerByBillingType(this.formData.consumerNumber).subscribe(
      (res: any) => {
        if (res.success == true) {
