...
export class HealthLogComponent implements OnInit {
  ...
  healthLogTimeInterval: number = environment.healthLogTimeInterval;
  ...
  ngOnInit(): void {
    ...
    currentDate.setDate(currentDate.getDate() - this.healthLogTimeInterval);
    this.formdata.fromdate = this.datePipe.transform(currentDate, 'yyyy-MM-dd');
  }

  onBtnExport() {
    var excelParams = {
      fileName: 'HealthLog.csv',
    };
