...
export class AlarmEventsComponent implements OnInit {
  ...
  datas: HeaderMenu = {
    ...
    levelurl: '',
    menuname: 'Alarm Events',
    url: '/mdm/assets/alarmevents',
  };
  ...
  constructor(
    ...
    this.gridOptions.getRowStyle = function (params) {
      if (params.node.data.isRead == false) {
  ...
  onGridReady(params: any) {
    ...
    this.service.getAlarmEvents(this.agridRequest).subscribe(
      (res: any) => {
        if (res.success) {
  ...
  getSelectedRowData() {
    ...
    let selectedData = selectedNodes.map((node) => node.data.hesEventID);
    let data = {
      id: selectedData.toString(),
      assignID: 0,
    };
    this.service.markAsReadNewEvents(data).subscribe(
      (res: any) => {
        if (res.success) {
