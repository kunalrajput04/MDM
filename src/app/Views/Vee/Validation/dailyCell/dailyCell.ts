...
export class MyDailyCellComponent implements OnInit, ICellRendererAngularComp {
  public params: any;
  public isNew: any;
  gridApi: any;
  meterSo = [];
  ...
  onBtnExport() {
    ...
    console.log(this.params?.data?.meterType,this.params.data.id)
   let m = this.params.data.meterType;
   let d = this.params.data.id
// if(this.params.data.MeterType !== 'Single phase'){
  
    this.veesr.singlePHDailyData({dailyLoadOutPutID:d,meterType:m}).subscribe((res:any)=>
    {
      res.map((a:any)=>{
        console.log(a.meterSno)
        this.meterSo=[...this.meterSo,a.meterSno];
      })
      console.log(res,{dailyLoadOutPutID:d,meterType:m},this.meterSo);
